* @ValidationCode : MjotNzYyMjczODA2OkNwMTI1MjoxNTk0NDkxODc4MTk5OnVzZXI6LTE6LTE6MDowOmZhbHNlOk4vQTpERVZfMjAxNzEwLjA6LTE6LTE=
* @ValidationInfo : Timestamp         : 12 Jul 2020 00:24:38
...
SUBROUTINE GB.EXIM.DEP.MA.15(Y.DATA)
*PROGRAM E.GB.EXIM.DEP.MA.15
*-----------------------------------------------------------------------------
*
*-----------------------------------------------------------------------------
...
*
    ...
    WHILE Y.AA.ID:POS
        ...
        Y.ARR.CO.CODE = REC.AA<AA.Framework.Arrangement.ArrCoCode>
*
*Y.ARR.STATUS EQ 'EXPIRED' OR Y.ARR.STATUS EQ 'PENDING.CLOSURE'
        IF Y.PRODUCT.LINE EQ "DEPOSITS" AND (Y.ARR.STATUS EQ 'EXPIRED' OR Y.ARR.STATUS EQ 'PENDING.CLOSURE') AND Y.AA.CUS.ID EQ Y.CUS.ID AND Y.ARR.CO.CODE EQ Y.COMP THEN
*
...
*-----------------------Principal--------------------------------
            ...
            Y.PAYMENT.DATE = EB.SystemTables.getToday()
