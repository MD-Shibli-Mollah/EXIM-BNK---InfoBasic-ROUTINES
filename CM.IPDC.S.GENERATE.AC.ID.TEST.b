* @ValidationCode : MjotNTI5NDg0NDY2OkNwMTI1MjoxNTcwMzY0MjY5MTM3OkRFTEw6LTE6LTE6MDowOmZhbHNlOk4vQTpERVZfMjAxNzEwLjA6LTE6LTE=
* @ValidationInfo : Timestamp         : 06 Oct 2019 18:17:49
...
* for IPDC, this routine allows to generate automatically the account number*
...
* account number with the following structure: I-CCC-PP-D-SSSSSS where:          *
* I: is the BRANCH.CODE of Interco.Parameter (the lead company of the company     *
*    where the user is currently logged in)                                       *
* CCC: is the company code of the company where the user is currently logged in   *
*       (First sub-value, multi-value correspondent to the current company) of the*
*       file  INTERCO.PARAMETER.                                                  *
* PP: Product Code entered by the User                    *
* SSSSSS: Sequentiel number obtained from Locking Table(F.LOCKING)                *
*         id (F<<MNE>>.ACCOUNT) ACCOUNT.NUMBER by                                 *
*         incrementing 1 to the existing number                                   *
* D: is the check digit calculated as specified by IPDC                     *
*=================================================================================*
...
SUBROUTINE CM.IPDC.S.GENERATE.AC.ID.TEST
    $INSERT I_COMMON
    $INSERT I_EQUATE
...
*=======
    ...
    EB.DataAccess.FRead(FN.COMPANY, COMPANY.ID, REC.COMPANY, F.COMPANY, COMPANY.ERR)
...
*FIN.COMPANY = 'GB0010001'
    ...
    FIND FIN.COMPANY IN Y.COMP.LIST<1,1> SETTING FIN.COM.POS1,FIN.COM.POS2,FIN.COM.POS3 THEN
        LEAD.COMPANY = FIN.COMPANY[4,2] 'R%2'
    END ELSE
        Y.COMP.POS = ''
    ...
    LOCKING.ID = 'ACCOUNT.':Y.PRODUCT.CODE
