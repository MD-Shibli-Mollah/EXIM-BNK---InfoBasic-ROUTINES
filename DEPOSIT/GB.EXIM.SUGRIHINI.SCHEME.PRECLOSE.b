* @ValidationCode : MjotMTYwMzU0NDU0MjpDcDEyNTI6MTU4MTQ4NjA5ODgwNjpERUxMOi0xOi0xOjA6MDpmYWxzZTpOL0E6REVWXzIwMTcxMC4wOi0xOi0x
* @ValidationInfo : Timestamp         : 12 Feb 2020 11:41:38
...
SUBROUTINE GB.EXIM.SUGRIHINI.SCHEME.PRECLOSE(arrId,arrProp,arrCcy,arrRes,balanceAmount,perDat)
*-----------------------------------------------------------------------------
*
*-----------------------------------------------------------------------------
...
*--------------------------------------------PREMATURE PROFIT----------------------------------------------
        CASE Y.DAYS GE 360 AND Y.DAYS LT 1080
            ...
            balanceAmount = (Y.ACT.PROFIT) - (Y.TOT.PROFIT+Y.TOT.PRINCIPAL.INT.AMT+AFTER.MATURE.PRIN)
*--------------------------------------------AFTER 3 YEARS PROFIT----------------------------------------------
        CASE Y.DAYS GE 1080 AND Y.DAYS LT 1800
            ...
            IF NUM.OF.REPAY GE 36 THEN
                Y.AA.INT.ID = 'EXIM.PFT.MSUMS3Y.DP-BDT--20040101'
...
*--------------------------------------------AFTER 5 YEARS PROFIT----------------------------------------------
        CASE Y.DAYS GE 1800 AND Y.DAYS LT 2880
            ...
            IF NUM.OF.REPAY GE 60 THEN
                Y.AA.INT.ID = 'EXIM.PFT.MSUMS5Y.DP-BDT--20040101'
                GOSUB GET.PRD.INTEREST.RATE
                NO.OF.LOOP = 60
                START.FORM.I = 1
                GOSUB GET.BASIC.DETAILS.WITH.SLAB
...
*--------------------------------------------AFTER 8 YEARS PROFIT----------------------------------------------
        CASE Y.DAYS GE 2880 AND Y.DAYS LT 3600
            ...
            IF NUM.OF.REPAY GE 96 THEN
                Y.AA.INT.ID = 'EXIM.PFT.MSUMS8Y.DP-BDT--20040101'
                GOSUB GET.PRD.INTEREST.RATE
                NO.OF.LOOP = 96
                START.FORM.I = 1
                GOSUB GET.BASIC.DETAILS.WITH.SLAB
...
*--------------------------------------------AFTER 10 YEARS PROFIT----------------------------------------------
        CASE Y.DAYS GE 3600 AND Y.DAYS LT 4320
            ...
            IF NUM.OF.REPAY GE 120 THEN
                Y.AA.INT.ID = 'EXIM.PFT.MSUMS10Y.DP-BDT--20040101'
                GOSUB GET.PRD.INTEREST.RATE
                NO.OF.LOOP = 120
                START.FORM.I = 1
                GOSUB GET.BASIC.DETAILS.WITH.SLAB
...
*--------------------------------------------AFTER 12 YEARS PROFIT----------------------------------------------
        CASE Y.DAYS GE 4320 AND Y.DAYS LT 7200
            ...
            IF NUM.OF.REPAY GE 144 THEN
                Y.AA.INT.ID = 'EXIM.PFT.MSUMS12Y.DP-BDT--20040101'
                GOSUB GET.PRD.INTEREST.RATE
                NO.OF.LOOP = 144
                START.FORM.I = 1
                GOSUB GET.BASIC.DETAILS.WITH.SLAB
