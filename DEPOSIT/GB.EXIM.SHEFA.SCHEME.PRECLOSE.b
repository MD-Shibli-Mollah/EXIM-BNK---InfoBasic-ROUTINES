* @ValidationCode : MjotNzU3NzM2NTYyOkNwMTI1MjoxNTgxNTA3NjIwNzY5OkRFTEw6LTE6LTE6MDowOmZhbHNlOk4vQTpERVZfMjAxNzEwLjA6LTE6LTE=
* @ValidationInfo : Timestamp         : 12 Feb 2020 17:40:20
...
SUBROUTINE GB.EXIM.SHEFA.SCHEME.PRECLOSE(arrId,arrProp,arrCcy,arrRes,balanceAmount,perDat)
*-----------------------------------------------------------------------------
*
*-----------------------------------------------------------------------------
...
*--------------------------------------------AFTER 3 YEARS PROFIT----------------------------------------------
        CASE Y.DAYS GE 1080 AND Y.DAYS LT 1800
            ...
            IF NUM.OF.REPAY GE 36 THEN
                Y.AA.INT.ID = 'EXIM.PFT.ESHMS3Y.DP-BDT--20040101'
...
*--------------------------------------------AFTER 5 YEARS PROFIT----------------------------------------------
        CASE Y.DAYS GE 1800 AND Y.DAYS LT 2880
            ...
            IF NUM.OF.REPAY GE 60 THEN
                Y.AA.INT.ID = 'EXIM.PFT.ESHMS5Y.DP-BDT--20040101'
                GOSUB GET.PRD.INTEREST.RATE
                NO.OF.LOOP = 60
                START.FORM.I = 1
                GOSUB GET.BASIC.DETAILS.WITH.SLAB
...
*--------------------------------------------AFTER 8 YEARS PROFIT----------------------------------------------
        CASE Y.DAYS GE 2880 AND Y.DAYS LT 3600
            ...
            IF NUM.OF.REPAY GE 96 THEN
                Y.AA.INT.ID = 'EXIM.PFT.ESHMS8Y.DP-BDT--20040101'
                GOSUB GET.PRD.INTEREST.RATE
                NO.OF.LOOP = 96
                START.FORM.I = 1
                GOSUB GET.BASIC.DETAILS.WITH.SLAB
