* @ValidationCode : Mjo5Mzg3Mzg5NjQ6Q3AxMjUyOjE1ODEyNDE0OTg2NjE6REVMTDotMTotMTowOjA6ZmFsc2U6Ti9BOkRFVl8yMDE3MTAuMDotMTotMQ==
* @ValidationInfo : Timestamp         : 09 Feb 2020 15:44:58
...
SUBROUTINE GB.EXIM.ZIADAH.TAX.CALC(arrId,arrProp,arrCcy,arrRes,balanceAmount,perDat)
*-----------------------------------------------------------------------------
*
*-----------------------------------------------------------------------------
...
*--------------------------------------------PREMATURE PROFIT----------------------------------------------
            CASE Y.DAYS GE 360 AND Y.DAYS LT 1080
                ...
                TOT.ACC.AMT = (Y.TOT.PROFIT+Y.TOT.PRINCIPAL.INT.RUHAMA+AFTER.RUHAMA.PRIN) - Y.TOT.REPAY.PRINCIPAL
                ...
                END
*--------------------------------------------AFTER 3 YEARS RUHAMA PROFIT----------------------------------------------
            CASE Y.DAYS GE 1080 AND Y.DAYS LT 1800
                GOSUB ACTUAL.PROFIT
                Y.REPAY.REF.TOT = R.AA.AC.REC<AA.PaymentSchedule.AccountDetails.AdRepayReference>
                NUM.OF.REPAY = DCOUNT(Y.REPAY.REF.TOT,VM)
                IF NUM.OF.REPAY GE 36 THEN
                    Y.AA.INT.ID = 'EXIM.PFT.ERUHS3Y.DP-BDT--20040101'
...
SLAB.PROFIT.CALCULATION:
    ...
    EndDate = Y.TODAY
    Rates = Y.INT.RATE
    BaseAmts = Y.TOT.PRINCIPAL.INT.RUHAMA
    InterestDayBasis = 'A'
    Ccy = 'BDT'
    RoundAmts = 0
    IntAmts = 0
...
GET.BASIC.DETAILS.WITH.SLAB:
    FOR I = START.FORM.I TO NO.OF.LOOP
        ...
        Y.TOT.PRINCIPAL.INT.RUHAMA = Y.TOT.PRINCIPAL.INT.RUHAMA + Y.PER.REPAY.AMT + RoundAmts
        Y.TOT.PRINCIPAL = Y.TOT.PRINCIPAL + Y.PER.REPAY.AMT
        GOSUB SLAB.PROFIT.CALCULATION
*        Y.COUNT = Y.COUNT - 1
    NEXT I
    Y.TOT.PRINCIPAL.INT.RUHAMA = Y.TOT.PRINCIPAL.INT.RUHAMA + RoundAmts
*--------------------------------------------------------
*   here check mauturity amount Gt or not
* if maturity amt is GT then maturity amt should be first deposited double amount
    Prop.CLASS = 'TERM.AMOUNT'
    AA.Framework.GetArrangementConditions(Y.ARR.ID, Prop.CLASS, Idproperty, Effectivedate, Returnids, Returnconditions1, Returnerror)
    R.REC1 = RAISE(Returnconditions1)
    Y.FIRST.AMOUNT = R.REC1<AA.TermAmount.TermAmount.AmtAmount>
    Y.MATURE.RUHAMA.AMT = Y.FIRST.AMOUNT * 2
    IF Y.TOT.PRINCIPAL.INT.RUHAMA GT Y.MATURE.RUHAMA.AMT THEN
        Y.TOT.PRINCIPAL.INT.RUHAMA = Y.MATURE.RUHAMA.AMT
    END
*------------------------------------------------------------------
RETURN
GET.BASIC.DETAILS.NOT.SLAB:
    FOR I = START.FORM.I TO NO.OF.LOOP
        ...
        END
        AFTER.RUHAMA.PRIN = AFTER.RUHAMA.PRIN + Y.PER.REPAY.AMT
*        Y.COUNT = Y.COUNT - 1
    NEXT I
RETURN
